...
namespace ShoesShop
{
    public class Clients : Users
    {
        ...
        public Clients(string name, string login, string password)
        {
            this.name = name; this.login = login; this.password = password;
        }

        // Конструктор для паттерна
        public Clients(Timer t, TextBox info)
        {
        ...
        public void Enter(Guests g)
        {
            ...
            if (dt.Rows.Count != 0)
            {
                ...
                {
                    info.Text = $"Добро пожаловать Клиент - {name}"; info.ForeColor = Color.Green;
                    g.name = name;
                    t.Enabled = true;
                }
                else
